...
/**
 * 将 data URL 转为 File，用于上传
 * 使用原生 window.fetch 确保返回标准 Response（含 .blob()）
 */
...
/** 远程存储开启时使用服务端 COS 配置，无需前端密钥 */
async function remoteTxCOSFileUpload(file: File): Promise<string> {
  const base = typeof window !== 'undefined' ? window.location.origin : ''
  const form = new FormData()
  form.append(`file`, file)
  const res = await window.fetch(`${base}/api/upload/cos`, {
    method: `POST`,
    body: form,
  })
  if (!res.ok) {
...
async function txCOSFileUpload(file: File) {
  ...
  const { secretId, secretKey, bucket, region, path, cdnHost } = parsed && (parsed.secretId && parsed.secretKey)
    ? parsed
    : defaultTxCOSConfig

  if (!secretId || !secretKey || !bucket || !region) {
    throw new Error(`请在图床设置中配置腾讯云 COS，或部署到 Vercel 并配置 TENCENT_COS_* 环境变量后开启远程存储`)
  }
  ...
  try {
    const presignedUrl = await getSignedUrl(s3Client, command, { expiresIn: 300 })
    // 开发环境走 Vite 代理，避免浏览器直连 COS 的 CORS 限制
    const uploadUrl = typeof import.meta !== 'undefined' && import.meta.env?.DEV ? '/__cos_upload' : presignedUrl
    const isProxy = uploadUrl === '/__cos_upload'
    const response = await window.fetch(uploadUrl, {
      method: isProxy ? `POST` : `PUT`,
      headers: {
        'Content-Type': file.type,
        ...(isProxy ? { 'X-Presigned-Url': presignedUrl } : {}),
      },
      body: file,
    })
...
export async function fileUpload(content: string, file: File) {
  const imgHost = (await store.get(`imgHost`)) || `txCOS`
  if (!(await store.get(`imgHost`))) {
    await store.set(`imgHost`, `txCOS`)
  }
  switch (imgHost) {
    case `aliOSS`:
